...
def main_get_trades(escritura = True, sp500 = False):
    if escritura:
        ...
        for item in tqdm.tqdm(lista_companies):
            data = quandl.get('WIKI/' + item, start_date="2012-09-01", end_date="2015-12-31").reset_index()
            data['ticket'] = item
            data = data[['ticket', 'Date', 'Close', 'Volume']]
            data.columns = ['ticket', 'date', 'close', 'volume']
            data = data.sort_values('date', ascending=True)
            # data[['close_5', 'volume_5']] = data[['close', 'volume']].shift(periods=5, fill_value=0)
            # data[['close_60', 'volume_60']] = data[['close', 'volume']].shift(periods=60, fill_value=0)
            data[f'close_{periodo_corto}'] = data['close'].shift(periods=periodo_corto, fill_value=0)
            data[f'close_{periodo_largo}'] = data['close'].shift(periods=periodo_largo, fill_value=0)
            data[f'media_{periodo_corto}'] = round(data['close'] * porcentaje_corto + data[f'close_{periodo_corto}'] *
                                                   (1 - porcentaje_corto), 2)
            data[f'media_{periodo_largo}'] = round(data['close'] * porcentaje_largo + data[f'close_{periodo_largo}'] *
            ...
            data[f'var_{periodo_corto}'] = round((data['close'] / data[f'close_{periodo_corto}']) - 1, 4) * 100
            data[f'var_{periodo_largo}'] = round((data['close'] / data[f'close_{periodo_largo}']) - 1, 4) * 100
